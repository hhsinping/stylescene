...
get_sampling_map(array_f32_d tgt_dm_np, array_f32_d tgt_K_np,
                 ...
                 bool invalid_depth_to_inf, array_f32_d points_np) {
    ...
    int n_points = points_np.shape(0);
    ...
    std::vector<float> maps(n_points * 3,
                            float(-10));
    ...
    for (int idx = 0; idx < n_points; ++idx) {
		vec3_t xyz(points_np.data() + idx * 3 * 1, 3, 1);
		Eigen::Vector4f xyzh(xyz(0), xyz(1), xyz(2), 1);
		
        Eigen::Vector3f src_uvd = src_Ps[0] * xyzh;
        float proj_d = src_uvd(2);
        float src_wf = src_uvd(0) / proj_d;
        float src_hf = src_uvd(1) / proj_d;
		
        maps[idx * 3 + 0] = (2 * src_wf / (src_width ))  - 1;
        maps[idx * 3 + 1] = (2 * src_hf / (src_height )) - 1;
        maps[idx * 3 + 2] = proj_d;
	}
    return std::make_tuple(create_arrayN<float>(maps, {n_points, 3}),
